...
if ($approval_enabled && !$room_disabled && ($status & STATUS_AWAITING_APPROVAL))
{
  ...
  elseif (isset($action) && ($action == "more_info"))
  {
    ...
    else
    {
      ...
      if (!empty($info_user))
      {
        $value .= "\n" . get_vocab("by") . " $info_user"  . " $info_user" ;
      }
      $value .= "\n----\n";
      $value .= $info_text ;
    }
...
$sql3 = "select count(*) as c1 from mrbs.mrbs_users where level = 3 and name='". $_SESSION["UserName"] ."';";
...
if($row3[0]==1){
    ...
    if (!$series)
    {
      echo "<input type=button onClick=\"window.open('confirm_entry.php?id=$id&UserName=" . $_SESSION["UserName"] . "','mywindow2','width=450,height=250')\" value='Confirm Entry'>";
    } 
    if (!empty($repeat_id)  && !$series && $repeats_allowed)
    {
      echo " - ";
