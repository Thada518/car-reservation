...
const roleIcon = { admin: <Shield size={12} />, approver: <UserCheck size={12} />, user: <UserIcon size={12} /> };
    ...
    <AppLayout>
      <div className="space-y-4 md:space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-slate-900">จัดการผู้ใช้</h1>
            <p className="text-slate-500 text-xs md:text-sm mt-0.5">เพิ่ม ลบ และจัดการสิทธิ์ผู้ใช้</p>
          </div>
          <button onClick={openCreate}
            ...
            <Plus size={16} /><span className="hidden sm:inline">เพิ่มผู้ใช้</span>
          </button>
        </div>

        {showForm && (
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 md:p-6">
            <h2 className="font-semibold text-slate-800 mb-4 text-sm md:text-base">{editing ? 'แก้ไขผู้ใช้' : 'เพิ่มผู้ใช้ใหม่'}</h2>
        ...
        <div className="md:hidden space-y-3">
          {loading ? <p className="text-center text-slate-400 py-8">กำลังโหลด...</p> :
            filtered.map(u => (
              <div key={u.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">@{u.username} {u.employee_id && `· ${u.employee_id}`}</p>
                    {u.department && <p className="text-xs text-slate-400">{u.department}</p>}
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    <button onClick={() => openEdit(u)} className="p-2 hover:bg-blue-50 text-blue-600 rounded-lg"><Pencil size={14} /></button>
                    <button onClick={() => handleDelete(u)} className="p-2 hover:bg-red-50 text-red-500 rounded-lg"><Trash2 size={14} /></button>
                  </div>
                </div>
              </div>
            ))}
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  <tr key={u.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 text-slate-500 text-xs">{u.employee_id || '-'}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEdit(u)} className="p-1.5 hover:bg-blue-50 text-blue-600 rounded-lg"><Pencil size={13} /></button>
                        <button onClick={() => handleDelete(u)} className="p-1.5 hover:bg-red-50 text-red-500 rounded-lg"><Trash2 size={13} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
