...
export default function NewBookingPage() {
  ...
  const [form, setForm] = useState({
    vehicle_id: '', purpose: '', destination: '',
    passenger_count: '1', start_datetime: '', end_datetime: '', notes: '',
  });
  ...
  const checkAvailability = async () => {
    ...
    try {
      const r = await api.get('/vehicles/availability', { params: { start: form.start_datetime, end: form.end_datetime } });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/bookings', { ...form, vehicle_id: Number(form.vehicle_id), passenger_count: Number(form.passenger_count) });
    ...
    <AppLayout>
      <div className="max-w-2xl mx-auto space-y-4 md:space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-slate-900">จองรถ</h1>
            <p className="text-slate-500 text-xs md:text-sm">กรอกรายละเอียดการจองรถ</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 md:p-6 space-y-4 md:space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              เลือกรถ * {checkingAvail && <span className="text-blue-500 font-normal">กำลังตรวจสอบ...</span>}
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
              {(availability.length > 0 ? availability : vehicles).map(v => {
                const avail = availability.length > 0 ? v.available : true;
                return (
                  <button key={v.id} type="button" disabled={!avail}
                    ...
                    <span className="text-2xl flex-shrink-0">{vehicleTypeIcon[v.type]}</span>
